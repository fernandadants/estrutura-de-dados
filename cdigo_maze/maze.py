...
class Maze:
    ...
    def find_prize2(self) -> bool:
        pilha = [self.init_pos_player]  # Começa da posição inicial do jogador
        visitado = set()

        while pilha:
            linha, coluna = pilha.pop()

            if (linha, coluna) in visitado:
                continue
            visitado.add((linha, coluna))

            if self.find_prize((linha, coluna)):
                return True

            # Movendo o jogador para a nova posição
            self.mov_player((linha, coluna))

            # Verifica casas adjacentes (cima, baixo, esquerda, direita)
            for dl, dc in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
                nova_linha, nova_coluna = linha + dl, coluna + dc
                
                if 0 <= nova_linha < self.M.shape[0] and 0 <= nova_coluna < self.M.shape[1]:
                    if self.is_free((nova_linha, nova_coluna)) and (nova_linha, nova_coluna) not in visitado:
